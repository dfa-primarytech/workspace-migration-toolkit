...
async function upload(convert) {
  ...
  if (!file || !file.name.toLowerCase().endsWith('.pptx')) throw new Error('Please choose a .pptx file.');
  if (file.size > session.maxUploadBytes) throw new Error('This file exceeds the upload limit.');
  if (convert && (file !== selected || !sourceHash)) throw new Error('Please check this presentation first.');
...
async function perform(convert) {
  busy(true); $('status').textContent = convert ? 'Converting and checking your presentation…' : 'Checking your presentation…';
  try {
    const report = await upload(convert);
    const parent = convert ? $('result') : $('summary'); parent.replaceChildren();
    const p = document.createElement('p'); p.textContent = `${report.pages} slides · ${Object.values(report.assetCounts).reduce((a,b)=>a+b,0)} recovered files · ${report.warnings.length} items to review`; parent.append(p);
    ...
    if (!convert) { sourceHash = report.sourceSha256; selected = $('file').files[0]; $('convert').hidden = false; }
      ...
      if (report.url) link(report.url, 'Open in Google Slides', parent);
      if (report.folderUrl) link(report.folderUrl, 'Open recovered files in Drive', parent);
      $('convert').hidden = true;
    }
    download(report, parent);
    $('status').textContent = !convert ? 'Ready to convert. Review the findings above.' : report.status.startsWith('failed') ? 'Conversion did not finish. Check the report and any saved files before retrying.' : 'Conversion finished. Please review the presentation and report.';
